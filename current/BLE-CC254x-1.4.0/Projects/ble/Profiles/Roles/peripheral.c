 ...
 * GLOBAL VARIABLES
 */
uint8 allowed_address[B_ADDR_LEN] = {0x3C,0x47,0xA1,0x72,0x39,0xD0};
   //{0xE2, 0x6A, 0x67, 0x80, 0x07, 0x00};
/*********************************************************************
 * EXTERNAL VARIABLES
 */
    ...
    case GAP_LINK_ESTABLISHED_EVENT:
      {
        ...
        if ( pPkt->hdr.status == SUCCESS )
        {
          ...
          if(compareAddress (pPkt->devAddr, allowed_address ) == 1)
          {
...
uint8 compareAddress ( uint8 address_1[B_ADDR_LEN], uint8 address_2[B_ADDR_LEN] )
{
  int i = 0;
  for( i = 0; i < B_ADDR_LEN; i++)
  {
    uint8 temp1 = address_1[i];
    uint8 temp2  = address_2[i];
